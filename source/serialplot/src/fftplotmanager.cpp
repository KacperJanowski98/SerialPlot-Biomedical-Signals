...
FftPlotManager::FftPlotManager(QWidget* plotArea, PlotMenu* menu,
                         const FftStream* stream, QObject* parent) :
    QObject(parent)
{
    _stream = stream;
    ...
    connect(infoModel, &QAbstractItemModel::dataChanged,
                this, &FftPlotManager::onChannelInfoChanged);
    ...
    connect(stream, &FftStream::numChannelsChanged, this, &FftPlotManager::onNumChannelsChanged);
    connect(stream, &FftStream::dataAdded, this, &FftPlotManager::replot);
...
void FftPlotManager::construct(QWidget* plotArea, PlotMenu* menu)
{
    _menu = menu;
    _plotArea = plotArea;
    _autoScaled = true;
...
void FftPlotManager::onNumChannelsChanged(unsigned value)
{
...
void FftPlotManager::onChannelInfoChanged(const QModelIndex &topLeft,
...
void FftPlotManager::checkNoVisChannels()
{
...
void FftPlotManager::setMulti(bool enabled)
{
    isMulti = enabled;
...
void FftPlotManager::setupLayout(bool multiPlot)
{
...
Plot* FftPlotManager::addPlotWidget()
{
    auto plot = new Plot();
    ...
    if (isMulti)
    {
        connect(plot->axisWidget(QwtPlot::yLeft), &QwtScaleWidget::scaleDivChanged,
                this, &FftPlotManager::syncScales);
    }
...
void FftPlotManager::syncScales()
{
...
void FftPlotManager::addCurve(QString title, const XFrameBuffer* xBuf, const FrameBuffer* yBuf)
{
...
void FftPlotManager::_addCurve(QwtPlotCurve* curve)
{
...
void FftPlotManager::removeCurves(unsigned number)
{
    if (_stream != nullptr)     // not displaying snapshot
    {
        if (! isMulti)
...
unsigned FftPlotManager::numOfCurves()
{
    return curves.size();
}

Plot* FftPlotManager::plotWidget(unsigned curveIndex)
{
    if (isMulti)
    {
...
void FftPlotManager::showGrid(bool show)
{
    for (auto plot : plotWidgets)
    {
        plot->showGrid(show);
...
void FftPlotManager::showMinorGrid(bool show)
{
    for (auto plot : plotWidgets)
    {
...
void FftPlotManager::showLegend(bool show)
{
    for (auto plot : plotWidgets)
    {
...
void FftPlotManager::setLegendPosition(Qt::AlignmentFlag alignment)
{
    for (auto plot : plotWidgets)
    {
...
void FftPlotManager::showDemoIndicator(bool show)
{
    isDemoShown = show;
    for (auto plot : plotWidgets)
    {
...
void FftPlotManager::darkBackground(bool enabled)
{
    for (auto plot : plotWidgets)
    {
...
void FftPlotManager::setSymbols(Plot::ShowSymbols shown)
{
    showSymbols = shown;
    for (auto plot : plotWidgets)
    {
...
void FftPlotManager::setYAxis(bool autoScaled, double yAxisMin, double yAxisMax)
{
...
void FftPlotManager::setXAxis(bool asIndex, double xMin, double xMax)
{
...
void FftPlotManager::flashSnapshotOverlay()
{
    for (auto plot : plotWidgets)
    {
...
void FftPlotManager::setNumOfSamples(unsigned value)
{
    _numOfSamples = value;
    for (auto plot : plotWidgets)
    {
...
void FftPlotManager::setPlotWidth(double width)
{
    _plotWidth = width;
    for (auto plot : plotWidgets)
    {
...
void FftPlotManager::exportSvg(QString fileName) const
{
